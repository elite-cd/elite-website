...
const TwMenu = ({ items, isWhiteMenu }) => {
  ...
  const getSubMenu = (val) => {
    ...
    const defaultIndextText = "text-teal-400 disabled:text-black/30 lg:px-2 [&.active]:text-black/90"
    const defaultText = "p-0 text-white hover:text-teal-400 focus:text-teal-400 disabled:text-black/30 lg:px-2 [&.active]:text-black/90"
    const whiteMenuText = "p-0 text-black hover:text-teal-400 focus:text-teal-400 disabled:text-black/30 lg:px-2 [&.active]:text-black/90"
                  ...
                  >
                      <span class="[&>svg]:w-7">
                          <svg
                              ...
                              viewBox="0 0 24 24"
                              class={"h-10 w-10 " + (isWhiteMenu ? "text-black" : "text-white")}
                          >
                              <path
                                  fill-rule="evenodd"
                                  fill="currentColor"
